...
static int16_t merge(IEcoMemoryAllocator1* pIMem, int32_t* array, int32_t left, int32_t mid, int32_t right) {
    int32_t i, j, k;
    int32_t n1 = mid - left + 1;
    int32_t n2 = right - mid;

    // Создаем временные массивы через системный аллокатор
    int32_t* L = (int32_t*)pIMem->pVTbl->Alloc(pIMem, n1 * sizeof(int32_t));
    int32_t* R = (int32_t*)pIMem->pVTbl->Alloc(pIMem, n2 * sizeof(int32_t));

    // Проверка на успешное выделение памяти
    if (L == 0 || R == 0) {
        // Если память не выделилась, нужно освободить то, что успели выделить
        if (L) pIMem->pVTbl->Free(pIMem, L);
        if (R) pIMem->pVTbl->Free(pIMem, R);
        return ERR_ECO_OUTOFMEMORY; // Возвращаем код ошибки (нужно определить или использовать -1)
    }
...
static int16_t mergeSortRecursive(IEcoMemoryAllocator1* pIMem, int32_t* array, int32_t left, int32_t right) {
    int16_t result = ERR_ECO_SUCCESES;
    if (left < right) {
        int32_t mid = left + (right - left) / 2;

        // Сортируем первую и вторую половины
        result = mergeSortRecursive(pIMem, array, left, mid);
        if (result != ERR_ECO_SUCCESES) return result;
        
        result = mergeSortRecursive(pIMem, array, mid + 1, right);
        if (result != ERR_ECO_SUCCESES) return result;

        // Сливаем отсортированные половины
        result = merge(pIMem, array, left, mid, right);
    }
    return result;
}
 ...
 *
 * <сводка>
 *   Функция Sort (Merge Sort)
 * </сводка>
 *
 * <описание>
 *   Функция сортировки массива целых чисел алгоритмом слияния
 * </описание>
 *
 */
static int16_t ECOCALLMETHOD CEcoLab1_Sort(/* in */ IEcoLab1Ptr_t me, /* in */ int32_t* array, /* in */ int32_t size) {
    CEcoLab1* pCMe = (CEcoLab1*)me;
    ...
    /* Вызов рекурсивной функции сортировки.
       Используем аллокатор, сохраненный в экземпляре компонента (pCMe->m_pIMem) */
    return mergeSortRecursive(pCMe->m_pIMem, array, 0, size - 1);
}
/*
 *
 * <сводка>
